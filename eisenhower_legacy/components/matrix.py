    ...
    def update(self, task_manager):
        ...
        for task in task_manager.tasks:
            if task.urgent and task.important:
                self.quadrants['urgent_important'].append(task)
            elif not task.urgent and task.important:
                self.quadrants['not_urgent_important'].append(task)
            elif task.urgent and not task.important:
                self.quadrants['urgent_not_important'].append(task)
            else:
                self.quadrants['not_urgent_not_important'].append(task)

        # 색상 정보 가져오기
        colors = self._get_colors()

        # 매트릭스 그리기
        self._draw_matrix(colors)

    def _draw_matrix(self, colors):
        """매트릭스 그리기"""
        # 사분면 그리기
        self.ax.add_patch(
            Rectangle((0, 0), 3, 3, facecolor='#FFE4E1'))  # 긴급하고 중요
        self.ax.add_patch(
            Rectangle((3, 0), 3, 3, facecolor='#E6E6FA'))  # 긴급하지 않고 중요
        self.ax.add_patch(
            Rectangle((0, 3), 3, 3, facecolor='#F0FFF0'))  # 긴급하고 중요하지 않음
        # 긴급하지 않고 중요하지 않음
        self.ax.add_patch(Rectangle((3, 3), 3, 3, facecolor='#FFF0F5'))

        # 작업 표시
        for i, (quadrant, tasks) in enumerate(self.quadrants.items()):
            x = 1.5 if i % 2 == 0 else 4.5
            y = 4.5 if i < 2 else 1.5

            for j, task in enumerate(tasks):
                task_y = y - (j * 0.2)
                color = colors[j % len(colors)]
                self.ax.add_patch(
                    Rectangle((x-0.1, task_y-0.1), 0.2, 0.2, facecolor=color))
                self.ax.text(x, task_y, task.name, ha='center', va='center',
        ...
        # 제목 설정
        self.ax.set_title('아이젠하워 매트릭스', fontsize=self.font_size * 1.5)
